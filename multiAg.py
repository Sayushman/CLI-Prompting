...
def get_weather(city:str):
    url=f"https://wttr.in/%7B{city.lower()}%7D?format=%C+%t"
...
SYSTEM_PROMPT = """
    ...
    For every tool call wait for the observe step which is the ouputfrom the called tool.

    Rules: 
    - Strictly follow the given json output format.
    - Ouput run one step at a time.
    - The sequence of steps is START (Where user give an input),
    ...
    - run_command(cmd:str): Takes a system linux command as string and executes the commands on user's system 
                            and returns the output from the command.

    Example1:
    START: Hey, can you solve 2 + 3 * 5 / 10
    PLAN: { "step": "PLAN": "content": "Seems like user is interested in math problems" } 
    PLAN: { "step": "PLAN": "content": "Looking at the problem,we should solve this using BODMAS approach" } 
    PLAN: { "step": "PLAN": "content": "Yes, the BODMAS is the correct thing to be done here.." }
    PLAN: { "step": "PLAN": "content": "first, we must multiply 3 * 5" }
    ...
    PLAN: { "step": "PLAN": "content": "Let's see if we have any available tool from he list of available tools" } 
    PLAN: { "step": "PLAN": "content": "Great, we have get_weather tool available for this query " }
    PLAN: { "step": "PLAN": "content": "I need to call get_weather tool for delhi as input for city " }
    PLAN: { "step": "TOOL": "tool": "get_weather", "input": "delhi" }
    PLAN: { "step": "OBSERVE": "tool": "get_weather", "output": "The tempreture of delhi is cloudy with 20 C" }
...
class MyOutputFormat(BaseModel):
    ...
    content: Optional[str] = Field(None, description="The optionasl string content for the step  ")
